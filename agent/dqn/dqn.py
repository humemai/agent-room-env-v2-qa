...
class DQNAgent:
    ...
    def step_1(self, obsevations_room: list, greedy: bool) -> None:
        """Run one step of the agent.

        Args:
            obsevations_room: the observations from the environment
            greedy: whether to act greedily
        ...
        if self.agent_to_episodic:
            ...
            remaining_room_observations = obsevations_room["room"]
        ...
        for idx in range(num_mm_agents):
            ...
            action, q_values_ = select_action(
                ...
                dqn=self.dqn_mm,
                epsilon=self.epsilon,
                action_space=self.action_space_mm,
            )
